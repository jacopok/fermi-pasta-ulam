...
\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/cell-5-output-1.pdf}}

which we can transform, making use of the matrix \(U\), whose entries
are given by \[
U_{nm} = \sqrt{ \frac{2}{N} } \sin\left(  \frac{nm\pi}{N} \right)\,.
\] \(U\) is a symmetric, invertible matrix, and it is its own inverse:
\(UU = I_{N}\). We can show that \(U^{T} V U = \Omega\), where
\(\Omega\) is a diagonal matrix.

\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/cell-6-output-1.pdf}}

\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/cell-7-output-1.pdf}}
